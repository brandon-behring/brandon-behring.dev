...
- **Lab graph data repaired**: titles/years/authors now real and accurate
  to each linked arXiv ID; search works.
...
- [~] **A4. First demo to build.** Live at `/lab/research-graph/` —
  interactive citation graph (Cytoscape.js + fcose), 95 nodes / 51 edges,
  reproducibly generated by the research-kb citation pipeline +
  `rl_and_control/scripts/build_graph_export.py` (`research-kb#16`/`#20`
  closed). The curation overlay (textbook/DPG corrections) is folded into
  `rl_and_control/references/graph_overrides.json` and emitted by the exporter,
  so the site is a pure consumer — the post-export enrich script is gone.
  Cosmetic follow-ups F1 (favicon) + F2 (Cytoscape `hsl()`) resolved 2026-05-28.
  **Visible densification (1b)** — pre-arXiv classics/textbooks — remains,
  gated on corpus acquisition (`research-kb#21`).
